...
const MessageBoard = ({ socket, messages, setMessages, sendMessage, updateMessages }) => {
  if (!messages.length) {
    socket.on('message', message => { // problematic if there are no messages to fetch - will continue to create sockets; however, must create socket exactly once in order to begin populating messages on emit
